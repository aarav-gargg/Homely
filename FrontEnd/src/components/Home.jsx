import React from 'react';

const Home = () => {
  return (
    <>
      <div
        className="relative min-h-[75vh] w-full flex flex-col items-center justify-center bg-cover bg-center bg-no-repeat"
        style={{
          backgroundImage: `url('https://hips.hearstapps.com/hmg-prod/images/alpe-di-siusi-sunrise-with-sassolungo-or-langkofel-royalty-free-image-1623254127.jpg')`
        }}
      >
        <div className="absolute inset-0 bg-black opacity-50"></div>
        <div className="relative p-8 rounded-lg text-center z-10">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
            Find Your Perfect Room with Roomly
          </h1>
          <p className="text-base md:text-lg text-white mb-6">
            Discover a wide range of affordable and cozy rooms tailored to your needs.
            Whether you’re looking for a short-term stay or a long-term home, Roomly has you covered.
          </p>
          <a
            href="/search"
            className="bg-blue-400 text-black py-2 px-6 rounded-lg text-lg font-semibold hover:bg-blue-500 transition duration-300"
          >
            Start Your Search
          </a>
        </div>
      </div>
      <div className="categories">
        <h1 className="text-4xl font-bold mb-4 font-yusei mx-10 my-5">Top Categories</h1>
        <div className="w-full bg-gray-200 p-8">
          <div className="h-[200px] m-auto overflow-hidden relative w-auto my-5">
            <ul className="flex w-[calc(250px*14)] animate-scroll justify-center items-center gap-5">
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://www.travelandleisure.com/thmb/JIoqZXurmgjBU-aRjKthU7oKu8A=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/1-71d7208a004a48b7bc1617a7e77183ea.jpg')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                  Beaches
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ900b1rZvv46KNz0IhAVfMWtPLcCpsNA16Jg&s')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                  Mountains
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT0BnmRmLJMVr9p0a0en3tF0sNCmkQoaCZLWQ&s')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                  Cities
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQgOwzHSAFctQNxoNdAhGWsdeAsek0hCUttdA&s')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                  Deserts
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://theamaryllis.in/uploads/editor_images/penthouse-1_1684575236.webp')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                Penthouse
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://i0.wp.com/theluxurytravelexpert.com/wp-content/uploads/2018/02/LAUCALA-ISLAND-FIJI.jpg?ssl=1')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                Luxury Resorts
                </span>
              </li>
              <li
                className="w-[250px] h-[200px] relative bg-cover bg-center bg-no-repeat rounded-lg overflow-hidden shadow-lg"
                style={{
                  backgroundImage: `url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu4VppHnM4IVNP8_bQt1CGICtpKLbZERg3JQ&s')`
                }}
              >
                <span className="absolute bottom-0 w-full bg-black bg-opacity-75 text-white text-center py-1">
                Cottages
                </span>
              </li>
              
            </ul>
          </div>
        </div>
      </div>
    </>
  );
};

export default Home;
